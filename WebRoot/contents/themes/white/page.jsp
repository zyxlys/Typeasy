...
<div class="main">
  <div class="left">
    <div class="cont">
      <h2><a href="post-${c.cid }.htm">${c.title }</a></h2>
      <p class="entry_data">作者：<c:forEach items="${sessionScope.users }" var="user">
						<c:if test="${c.authorId == user.uid }">${user.screenName }</c:if>
					</c:forEach></p>
	  <div class="content">
        ${c.text }
        <p class="tag"><c:forEach items="${relationships }" var="relationship">
