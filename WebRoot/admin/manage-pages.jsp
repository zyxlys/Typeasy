				...
				<table class="tablesorter" cellspacing="0">
					...
					<c:if test="${content.type == 'page' }">
						<tr>
							...
							<td><c:forEach items="${sessionScope.users }" var="user"><c:if test="${content.authorId == user.uid }">${user.screenName }</c:if></c:forEach></td>
							<td>${article.created }</td>
							<td>
							<a href="servlet/ContentsServlet?action=find&cid=${content.cid}&type=page"><img src="images/icn_edit.png" alt="编辑" style="border:0px;"/></a>
							<a href="servlet/ContentsServlet?action=del&cid=${content.cid}&type=page" onclick="return confirm('确认删除？')"><img src="images/icn_trash.png" alt="删除" style="border:0px;"/></a></td>
						</tr>
						</c:if>
					</c:forEach>
					</tbody>
