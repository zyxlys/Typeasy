...
<section id="main" class="column">
	...
	<form action="servlet/ContentsServlet?action=multiDel&type=page"
		method="post">
		<fieldset style="width: 95%;margin: 20px 3% 0 3%;">
			<input type="checkbox" name="all" onclick="selectAll();" /> <select
				...
				<option value="multiDel">删除选中的页面</option>
		...
		<article class="module width_full">
								...
								<pg:item>
									<tr>
										<td><input type="checkbox" name="cid"
											value="${content.cid }"><c:choose><c:when test="${fn:length(content.title) > 15 }">${fn:substring(content.title,0,15) } ...</c:when><c:otherwise>${content.title }</c:otherwise></c:choose></td>
										<td><c:forEach items="${sessionScope.users }" var="user">
												<c:if test="${content.authorId == user.uid }">${user.screenName }</c:if>
											</c:forEach></td>
										<td>${content.created }</td>
										<td><a
											href="servlet/ContentsServlet?action=find&cid=${content.cid}&type=page"><img
												src="images/icn_edit.png" alt="编辑" style="border:0px;" /></a> <a
											href="servlet/ContentsServlet?action=del&cid=${content.cid}&type=page"
			...
			<div style="text-align: center;padding:10px;"><pg:first>
						<a href="${pageUrl}">首页</a>
					</pg:first>
					<pg:prev>
						<a href="${pageUrl}">上页</a>
